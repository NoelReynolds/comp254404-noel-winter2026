 ...
 */
public class ModifiedTreeMap<K,V> extends AbstractSortedMap<K,V> {
     ...
     */
    public ModifiedTreeMap(Comparator<K> comp) {
    ...
    private Position<Entry<K,V>> treeSearch(Position<Entry<K,V>> p, K key) {
        Position<Entry<K,V>> returnPos = p;

        //Retain early return from original tree
        if (isExternal(p))
            return p;                          // key not found; return the final leaf

        int comp = compare(key, p.getElement());

        //Modify the return position and 'comp' value until the closest value (or the target value) is reached
        while (comp != 0)
        {
            if (comp < 0 && !isExternal(returnPos))
            {
                comp = compare(key, left(returnPos).getElement());
                returnPos = left(returnPos);
            }
            else if (comp > 0 && !isExternal(returnPos))
            {
                comp = compare(key, right(returnPos).getElement());
                returnPos = right(returnPos);
            }
            else break;
        }
        return returnPos;
